...
public class charMove2 : MonoBehaviour
{
    ...
    public void attack()
    {
        ...
        Collider2D[] boss = Physics2D.OverlapCircleAll(attackPoint.transform.position, radius, enemies);

        foreach (Collider2D enemyGameObject in boss)
        {
            Boss bossController = enemyGameObject.GetComponent<Boss>();
        ...
        {
            EnemyControl enemyController = enemyGameObject.GetComponent<EnemyControl>();
